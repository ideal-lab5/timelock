...
//!
//! This module uses the production Quicknet chain public key from api.drand.sh
...
//!
//! **Validity period:** This key is valid as long as the Drand Quicknet chain
//! remains active and the public key is not rotated. Always verify the current public key at https://api.drand.sh/info before use.
//!
//! **Intended use:** TESTING ONLY. This key is hardcoded for use in test code
//! and examples. DO NOT USE THIS KEY IN PRODUCTION. Always fetch and verify the
...
// - Any additional small fields required by the protocol (e.g., flags, status
//   codes)
// The value is intentionally conservative to accommodate possible future
// changes or unanticipated metadata fields. Update as protocol details become
// more concrete.
...
const MAX_OVERHEAD_BYTES: usize = TIMELOCK_CIPHERTEXT_OVERHEAD +  // Core timelock overhead (shared with lib.rs)
    PROTOCOL_METADATA_SIZE +
    SAFETY_MARGIN;

// Overhead multiplier for very small messages is calculated based on documented
// measurements. For example, as verified in August 2025, encrypting a 1-byte
// payload resulted in:
// - AES-GCM: 44 bytes ciphertext (44x overhead)
// - libsodium secretbox: 49 bytes ciphertext (49x overhead)
// The multiplier is set to the maximum observed ratio, rounded up, to ensure
// tests do not fail due to unexpected overhead in edge cases. Update these
// values if future measurements indicate a different upper bound.
//
// IMPORTANT: These values should be periodically re-validated to ensure they
// accurately reflect the maximum observed overhead. Re-validation should be
// performed:
// - After updating cryptographic libraries or dependencies (e.g., AES-GCM,
//   libsodium).
...
//
// To re-validate:
// 1. Encrypt a 1-byte payload using each supported cryptographic backend (e.g.,
//    AES-GCM, libsodium secretbox).
// 2. Measure the resulting ciphertext size for each backend.
// 3. Update AES_GCM_CIPHERTEXT_SIZE and LIBSODIUM_SECRETBOX_CIPHERTEXT_SIZE to
//    the maximum observed values.
// 4. Ensure that MAX_OVERHEAD_MULTIPLIER is set to the highest ratio
//    (ciphertext size / plaintext size), rounded up.
// 5. If any observed ciphertext size exceeds the current constants, update the
//    constants and re-run all tests.
//
// Indicators that values need updating:
// - Observed ciphertext sizes in tests exceed the hardcoded constants.
// - Release notes or documentation for cryptographic libraries mention changes
//   to overhead or output format.
...
const fn calculate_max_overhead_multiplier(
	aes_gcm_size: usize,
	secretbox_size: usize,
	min_msg_size: usize,
) -> usize {
	...
	// Therefore, we use this manual formula to compute the ceiling of integer
	// division in a way that is compatible with const evaluation.
	let aes_gcm_mult = (aes_gcm_size + min_msg_size - 1) / min_msg_size;
	let secretbox_mult = (secretbox_size + min_msg_size - 1) / min_msg_size;
	if aes_gcm_mult > secretbox_mult {
		aes_gcm_mult
	} else {
		secretbox_mult
	}
...
const MAX_REASONABLE_OVERHEAD_BYTES: usize =
	TIMELOCK_CIPHERTEXT_OVERHEAD + PROTOCOL_METADATA_SIZE + SAFETY_MARGIN;
...
/// Helper function to validate size estimation overhead for different message
/// sizes
///
/// For very small messages (1-4 bytes), we use a fixed maximum size check
/// because the overhead multiplier would be extremely high due to fixed
/// cryptographic headers. For small messages (5-127 bytes), we use the
/// multiplier-based check. For larger messages (128+ bytes), we use a fixed
/// overhead check.
fn validate_size_estimation_overhead(msg_len: usize, estimated: usize) {
	if msg_len <= VERY_SMALL_MESSAGE_THRESHOLD {
		// For very small messages (1-4 bytes), just check that overhead is reasonable
		// (under 500 bytes total) This threshold was chosen because the fixed
		// cryptographic headers (188 bytes) dominate the overhead
		assert!(
...
fn test_identity_creation_null_buffer() {
	let result = unsafe { timelock_create_drand_identity(1000, ptr::null_mut(), 32) };
	assert_eq!(result, TimelockResult::InvalidInput);
}
...
fn test_decrypt_invalid_inputs() {
	...
	let result = unsafe {
		timelock_decrypt(ptr::null(), sig_hex.as_ptr(), plaintext.as_mut_ptr(), &mut plaintext_len)
	};
	assert_eq!(result, TimelockResult::InvalidInput);
	...
	// Test null signature
	let result = unsafe {
		timelock_decrypt(&fake_ciphertext, ptr::null(), plaintext.as_mut_ptr(), &mut plaintext_len)
	};
	assert_eq!(result, TimelockResult::InvalidInput);
}
...
fn test_memory_management() {
	...
	let ciphertext = Box::new(TimelockCiphertext { data: data_ptr, len: data_len });
...
fn test_error_messages_after_failure() {
	unsafe {
		...
		if !error_ptr.is_null() {
			...
			assert!(
				error_str.to_lowercase().contains("null") ||
...
fn test_decrypt_buffer_size_handling() {
	...
	let ciphertext = TimelockCiphertext { data: data_ptr, len };
...
fn test_estimate_size_boundary_conditions() {
	unsafe {
		...
		for msg_len in [1, 16, 64, 256, 1024, 4096].iter() {
			...
			// For small messages, timelock has significant overhead due to fixed headers
			// and metadata.
...
fn test_cryptographic_constants_match_library() {
	// Validate that our engine-derived BLS constants match the actual library
	// values This test ensures that if the ark-bls12-381 library changes its
	// serialization format, we'll be alerted to update our constants.
	...
	assert!(
		crate::SERIALIZATION_OVERHEAD > 0,
		"SERIALIZATION_OVERHEAD ({}) should be positive",
		crate::SERIALIZATION_OVERHEAD
	);
	assert!(
		crate::SERIALIZATION_OVERHEAD < 50,
		"SERIALIZATION_OVERHEAD ({}) seems unreasonably large for metadata",
		crate::SERIALIZATION_OVERHEAD
	);

	// 16 bytes is reasonable for: Vec length encoding (8 bytes) + cipher_suite
	// length (8 bytes) + misc This validates that our constant is in a sensible
	// range for serialization metadata
	assert!(
		crate::SERIALIZATION_OVERHEAD >= 8 && crate::SERIALIZATION_OVERHEAD <= 32,
